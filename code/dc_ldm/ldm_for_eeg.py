...
class cond_stage_model(nn.Module):
    def __init__(self, metafile, num_voxels=440, cond_dim=1280, global_pool=True, clip_tune = True, cls_tune = False):
        ...
        if clip_tune:
            self.data = torch.load('datasets/eeg_5_95_std.pth')
    ...
    # Cosine Similarity
    def cos_sim(self, x, image_embeds):
        target_emb = self.mapping(x)
        loss = 1 - torch.cosine_similarity(target_emb, image_embeds, dim=-1).mean()
        return loss

    # Dot-Product Similarity
    def dot_sim(self, z1, z2):
        z1_normed = z1 / torch.norm(z1)
        z2_normed = z2 / torch.norm(z2)
        return torch.dot(z1_normed, z2_normed)
    ...
    def simclr_loss(self, raw_eegs, img_embs, tau=0.10, sim='cos'):
        ...
        for i in range(batch_size):
            for j in range(batch_size):
                if sim == 'cos':
                    all_sims[i,j] = torch.exp(torch.cosine_similarity(eeg_embs[i,:], img_embs[j,:], dim=-1) / tau)
                else:
                    all_sims[i,j] = torch.exp(self.dot_sim(eeg_embs[i,:], img_embs[j,:]) / tau)
    ...
    def scl_loss(self, raw_eegs, img_embs, labels, tau=0.10, sim='cos'):
        eeg_embs = self.mapping(raw_eegs)
        # eeg_embs: Shape(8, 768) dtype=float32
        # img_embs: Shape(7, 768) dtype=float32
        # labels: Shape(8), dtype=int64
        batch_size = len(img_embs)
...
class eLDM:
    ...
    def finetune(self, trainers, dataset, test_dataset, bs1, lr1,
        ...
        print('\n##### Stage One: only optimize conditional encoders #####')
        dataloader = DataLoader(dataset, batch_size=bs1, num_workers=24, shuffle=True)
        test_loader = DataLoader(test_dataset, batch_size=bs1, num_workers=24, shuffle=False)
    ...
    def generate(self, fmri_embedding, num_samples, ddim_steps, HW=None, limit=None, state=None, output_path = None):
        ...
        with model.ema_scope():
            ...
            for count, item in enumerate(fmri_embedding):
                ...
                #     with open(labels_file, 'a' if os.path.exists(labels_file) else 'w') as file:
                #         file.write(str(item['label'].item()) + '\n')
                latent = item['eeg']

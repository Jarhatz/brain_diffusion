...
class LatentDiffusion(DDPM):
    ...
    def forward(self, x, c, labels, image_raw, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                ...
                # from get_learned_conditioning
                # C input: torch.Size([8, 128, 512])
                # C output: torch.Size([8, 77, 768])
                # re_latent: torch.Size([8, 128, 1024])
        ...
        if self.clip_tune:
            img_embs = self.image_embedder(image_raw)
            # loss_clip = self.cond_stage_model.cos_sim(re_latent, img_embs) # avg_loss: 0.5  time: 0.0006
            loss_clip = self.cond_stage_model.simclr_loss(re_latent, img_embs, sim='cos') # avg_loss: 2.0  time: 0.09
            # loss_clip = self.cond_stage_model.scl_loss(re_latent, img_embs, labels, sim='cos')
        ...
        if self.cls_tune:
            pre_cls = self.cond_stage_model.get_cls(re_latent)
            loss_cls = self.cls_loss(labels, pre_cls)
